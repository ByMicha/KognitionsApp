...
const TARGET_SEQUENCE = [63, 56, 49, 42, 35]; // 70 minus 7er-Schritte
...
export default function MocaCalculation({ theme, onComplete }) {
  ...
  // Erzeugt 1 korrekte und 3 falsche Antworten
  const generateOptions = (correctValue) => {
    let choices = [correctValue];
    while (choices.length < 4) {
      const offset = [1, -1, 10, -10, 7, -7, 2, -2][Math.floor(Math.random() * 8)];
  ...
  const handleSelection = (selected) => {
    ...
    if (currentIndex < TARGET_SEQUENCE.length - 1) {
      ...
      // Aufgabe beendet: Buttons verstecken & Haupt-Button freischalten
      setIsFinished(true);
      onComplete(newResults);
    }
  };

  // 1. INSTRUKTIONS-SCREEN
  if (!isStarted) {
    return (
      <View style={styles.container}>
        ...
        <View style={styles.explanationArea}>
          ...
          <Text style={styles.desc}>
            ...
            Wählen Sie für jeden Schritt das richtige Ergebnis aus.
          </Text>
          <TouchableOpacity 
    ...
    <View style={styles.container}>
      <View style={styles.textContainer}>
        ...
        <Text style={styles.desc}>
          {isFinished 
            ...
            : `Rechnen Sie: ${currentIndex === 0 ? "70" : TARGET_SEQUENCE[currentIndex - 1]} minus 7`}
        </Text>
      </View>
      ...
      <View style={styles.centerArea}>
        ...
        <View style={styles.calcGrid}>
          {[0, 1, 2, 3, 4].map((i) => {
            const res = results[i];
            const isCurrent = currentIndex === i && !isFinished;
            return (
              <View 
                key={i} 
                style={[
        ...
        {/* Antwortmöglichkeiten verschwinden nach Abschluss */}
        {!isFinished ? (
          ...
          <View style={styles.successArea}>
            ...
            <Text style={styles.successText}>Ergebnisse gespeichert ✓</Text>
            <Text style={styles.successSubText}>Klicken Sie nun unten auf "Nächster Test".</Text>
          </View>
        )}
      </View>
    </View>
...
const styles = StyleSheet.create({
  ...
  successText: { color: '#2ecc71', fontWeight: 'bold', fontSize: 20, marginTop: 10 },
  successSubText: { color: '#888', fontSize: 14, marginTop: 5 }
});